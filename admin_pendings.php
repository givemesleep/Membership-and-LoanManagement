  ...
  <div class="pagetitle">
      ...
      <nav>
          <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Member Pendings</li>
          </ol>
      </nav>
  </div><!-- End Page Title -->
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title" style="font-size: 35px;"><b>Membership Pending</b></h5>
              <table class="table datatable table-hover" style="width: 100%;">
                  ...
                  <tbody>                                    
                      <?php
                          ...
                          $sql = "SELECT 
                                  p.memberID AS memberID , CONCAT(IF(p.gendID = 2, 'Mr. ', 'Ms. '), p.memGiven, ' ', p.memSur) AS Fullname,
                                  ms.marDep AS Marital, st.memstats AS memStatus, p.memDOB AS bday, memaddr AS Address
                                  FROM tbperinfo p
                                  JOIN tbmaritals ms ON p.maritID = ms.maritID
                                  JOIN tbmemstats st ON p.memstatID = st.memstatID
                                  JOIN tbconinfo cn ON p.memberID = cn.memberID
                                  WHERE p.isActive = 0 AND p.memstatID = 2 OR p.memstatID = 3
                                  ORDER BY p.memSur AND p.memstatID = 2 ASC
                            ...
                            while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                              $btn = '
                              <a href="admin_pendview.php?viewmem='.encrypt($row['memberID'], $key).'"><button class="btn btn-dark" title="View"><i class="ri-eye-fill"></i></button></a>
                              ';

                              // $span = '<span class="badge bg-secondary">New</span>';

                              if($row['memStatus'] == 'Pending'){
                                $span = '<span class="badge bg-warning">Pending</span>';
                              }else{
                                $span = '<span class="badge bg-info">On-Going</span>';
                              }
                              ...
                              $disp .= '<tr>
                                          ...
                                          <td>'. bday($row['bday']) .'</td>
                                          <td>'. bday($row['bday']) .'</td>
                                          <td class="align-middle text-center"><h5>'.$span.'</h5></td>
                                          <td>
                                          '. $btn .'
                                          </td>
                                        </tr>';
